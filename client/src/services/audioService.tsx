...
export const removeFollow = async (targetUserId: string, token: string) => {

  const response =  await fetch(`${import.meta.env.VITE_APP_API_URL}/unFollow/${targetUserId}`, {
    method: "DELETE",
    headers: {
      "Authorization": `Bearer ${token}`
    }
...
export const getFollowers = async (targetUserId: string, token: string) => {

  const response = await fetch(`${import.meta.env.VITE_APP_API_URL}/followers/${targetUserId}`, {
    method: "GET",
    headers: {
      "Authorization": `Bearer ${token}`
    }
